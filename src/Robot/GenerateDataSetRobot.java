 ...
 */
public class GenerateDataSetRobot extends AdvancedRobot {
    private class Dados {
        boolean hit = false;
        String robotName;
        double power;
        ...
        public Dados(String robotName, double power, double energy, double bearing, double distance, double robotHeading, double gunHeading, double velocity) {
    ...
    public void onScannedRobot(ScannedRobotEvent e) {
        ...
        if (getEnergy() > bulletPower) {
            bullet = this.fireBullet(bulletPower);
            if (bullet != null) {
                Dados d = new Dados(e.getName(), bulletPower, e.getEnergy(), e.getBearing(), e.getDistance(),
                        e.getHeading(), this.getGunHeading(), e.getVelocity());
                balasNoAr.put(bullet, d);
            }
        }
    }
    ...
    public void onBulletHit(BulletHitEvent event) {
        ...
        if (event.getName().equals(event.getBullet().getVictim())) {
            GenerateDataSetRobot.Dados d = balasNoAr.get(event.getBullet());
            d.hit = true;
            balasNoAr.replace(event.getBullet(), d);
        }
    }
    ...
    public void onRoundEnded(RoundEndedEvent event) {
        ...
        RobocodeFileWriter robotWriter = null;
        BufferedWriter bufferedWriter = null;

        try {
            robotWriter = new RobocodeFileWriter(getDataFile("dataSetRobot.csv"));
            bufferedWriter = new BufferedWriter(robotWriter);

            Iterator it = balasNoAr.entrySet().iterator();
            while (it.hasNext()) {
                Map.Entry pair = (Map.Entry) it.next();
                Dados dados = balasNoAr.get(pair.getKey());
                String data = dados.robotName + "," + dados.hit + "," + dados.power + "," + dados.energy + "," + dados.bearing
                        + "," + dados.distance + "," + dados.robotHeading + "," + dados.gunHeading + "," + dados.velocity + "\n";
